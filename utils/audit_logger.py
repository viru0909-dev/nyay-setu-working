...
try:
    ...
    logging.error(
        f"Failed to initialize file audit logger at {AUDIT_LOG_PATH}: {e}"
    )


def log_document_access(user_id, document_id, action):
    """
    Logs document access events asynchronously to avoid blocking the main request thread.  # noqa
    """
    try:
        ...
        logging.error(
            f"Failed to trigger audit log for document {document_id}: {e}"
        )
...
def _write_audit_log(app, audit_log):
    ...
    try:
        with app.app_context():
            ...
            if db is not None:
                # Insert a copy so Mongo's inserted _id doesn't mutate the dictionary  # noqa
                db.audit_logs.insert_one(audit_log.copy())
            else:
                logging.error(
                    "MongoDB audit logging failed: MONGO_DB not found in app config"  # noqa
                )
    except Exception as e:
    ...
    try:
        ...
        # 5. Error Handling: log error but continue request, handle permission errors  # noqa
