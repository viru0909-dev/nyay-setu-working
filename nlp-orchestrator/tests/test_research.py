...
async def test_retry_then_success(monkeypatch):
    ...
    with patch("research.build_provider_queue", return_value=["groq"]), patch(
        "research._attempt_provider", mock_attempt
    ), patch("research.RETRY_DELAY_SECONDS", 0), patch(
        "research.RETRY_MAX_ATTEMPTS", 1
    ), patch(
        "research.is_retryable_exception", return_value=True
    ), patch(
    ...
    with patch("research.build_provider_queue", return_value=["gemini", "groq"]), patch(
        ...
        AsyncMock(
            side_effect=AssertionError(
                "Gemini should not be called when breaker is open"
            )
        ),
    ), patch(
        "research._call_groq_once", AsyncMock(return_value=ok)
    ):
...
async def test_non_retryable_error(monkeypatch):
    ...
    with patch("research.build_provider_queue", return_value=["groq"]), patch(
        "research._attempt_provider", mock_attempt
    ), patch("research.is_retryable_exception", return_value=False), patch(
        "research.RETRY_DELAY_SECONDS", 0
    ):
