...
CITATION_PATTERNS = [
    re.compile(
        r"\b(?P<act>IPC|CrPC|CPC|MVA|BNS|BNSS|Article|Art|Indian Penal Code|Criminal Procedure Code|Civil Procedure Code|Motor Vehicles Act)\s*(?:Section|Sec|S\.?|Article|Art)?\.?\s*(?P<section>\d{1,4}[A-Za-z]?)\b",  # noqa
        re.IGNORECASE,
    ),
    re.compile(
        r"\b(?:u/s|under section|section|sec|s\.?|article|art)\s*(?P<section>\d{1,4}[A-Za-z]?)\s*(?:of\s*)?(?P<act>IPC|CrPC|CPC|MVA|BNS|BNSS|Article|Art)\b",  # noqa
        re.IGNORECASE,
    ),
]
