...
All API keys and secrets MUST be provided via environment variables or a .env file.  # noqa
Never hardcode secrets in source code — use .env.example as a template.
All new variables have safe defaults; existing .env files keep working unchanged.  # noqa
"""
...
# ─── API keys — loaded from environment variables only ────────────────────────  # noqa
...
# ─── Validate required keys ───────────────────────────────────────────────────  # noqa
if not GROQ_API_KEY:
    print(
        "[Config] ERROR: GROQ_API_KEY is not set. Please add it to .env (see .env.example)",  # noqa
        file=sys.stderr,
    )
    sys.exit(1)
if not GEMINI_API_KEY:
    print(
        "[Config] WARNING: GOOGLE_GEMINI_API_KEY not set. Gemini calls will fall back to Groq."  # noqa
    )
