    ...
    public ResponseEntity<?> issueSummons(@PathVariable UUID id, Authentication authentication) {
        try {
            ...
            if (caseEntity.getJudge() == null || !caseEntity.getJudge().getId().equals(judge.getId())) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Access Denied: You are not authorized to issue summons for this case as it is outside your assigned jurisdiction");
            }
