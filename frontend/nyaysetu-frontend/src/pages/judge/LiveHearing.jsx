...
export default function LiveHearing() {
    ...
    // --- Captioning Feature State (Issue #1322) ---
    ...
    // --- Audio Extraction & WebSocket Logic (Issue #1322) ---
    useEffect(() => {
    ...
    }, [activeHearing, isCaptioningEnabled, targetLanguage]);
    ...
    if (activeHearing) {
        return (
            <div style={{ maxWidth: '1400px', margin: '0 auto', paddingTop: '2rem', paddingBottom: '2rem' }}>
                <div style={{ height: '75vh', display: 'flex', flexDirection: 'column', background: '#0a0a0f', borderRadius: '1.5rem', overflow: 'hidden', border: '2px solid rgba(99, 102, 241, 0.3)', boxShadow: '0 20px 60px rgba(0,0,0,0.4)', position: 'relative' }}>
                    
                    {/* Video Header Area */}
                    <div style={{ padding: '1rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(17, 24, 39, 0.95)', borderBottom: '1px solid rgba(99, 102, 241, 0.2)', zIndex: 10 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                            <button onClick={endCall} aria-label="Leave hearing" style={{ background: 'rgba(239, 68, 68, 0.1)', border: 'none', color: '#ef4444', padding: '0.5rem', borderRadius: '0.5rem', cursor: 'pointer' }}>
                                <ArrowLeft size={20} />
                            </button>
                            <div>
                    ...
                    <div style={{ flex: 1, position: 'relative', background: '#1a1a1f', minHeight: 0 }}>
                        <iframe
                            src={`https://meet.jit.si/${activeHearing.videoRoomId || 'nyaysetu-court-' + activeHearing.id}#config.prejoinConfig.enabled=false&config.startWithAudioMuted=false&config.startWithVideoMuted=false`}
                            style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', border: 'none' }}
                            allow="camera; microphone; fullscreen; display-capture; autoplay"
                            title="NyaySetu Court Hearing"
                        />
                        
                        {/* --- Scalable Caption Overlay (Issue #1322) --- */}
                        {isCaptioningEnabled && captions && (
                            <div className="absolute bottom-6 left-0 right-0 flex justify-center px-4 z-50 pointer-events-none" style={{ position: 'absolute', bottom: '20px', left: 0, right: 0, display: 'flex', justifyContent: 'center', zIndex: 50, pointerEvents: 'none' }}>
                                <div style={{ background: 'rgba(0, 0, 0, 0.85)', color: 'white', fontSize: '1.25rem', padding: '0.75rem 1.5rem', borderRadius: '0.75rem', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.5)', maxWidth: '80%', textAlign: 'center', fontWeight: '500', border: '1px solid rgba(255,255,255,0.1)' }}>
                                    {captions}
                                </div>
                            </div>
                        )}
                    ...
                    {/* Bottom Control Bar */}
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem 1.5rem', background: 'rgba(17, 24, 39, 0.95)', borderTop: '1px solid rgba(99, 102, 241, 0.2)', zIndex: 10 }}>
                        
                        {/* --- Language Toggle & Controls (Issue #1322) --- */}
                        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                            <button 
                                onClick={() => setIsCaptioningEnabled(!isCaptioningEnabled)}
                                style={{ padding: '0.6rem 1.2rem', borderRadius: '0.5rem', color: 'white', fontWeight: '600', border: 'none', cursor: 'pointer', background: isCaptioningEnabled ? '#ef4444' : '#3b82f6', transition: 'background 0.2s' }}
                            >
                                {isCaptioningEnabled ? "Disable Captions" : "Enable Captions"}
                            </button>
                            ...
                            {isCaptioningEnabled && (
                                <select 
                                    ...
                                    style={{ padding: '0.6rem', background: '#1f2937', color: 'white', border: '1px solid #374151', borderRadius: '0.5rem', outline: 'none', cursor: 'pointer' }}
                                >
                        ...
                        <button onClick={endCall} style={{ padding: '0.75rem 2rem', background: 'linear-gradient(135deg, #ef4444 0%, #b91c1c 100%)', border: 'none', borderRadius: '9999px', color: 'white', fontWeight: '800', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.75rem', boxShadow: '0 10px 25px rgba(239, 68, 68, 0.4)', transition: 'all 0.2s', letterSpacing: '0.5px' }}>
                            <Phone size={18} />
                            END SESSION
                        </button>
                    </div>
    ...
    return (
        <div style={{ maxWidth: '1400px', margin: '0 auto', paddingBottom: '2rem' }}>
            <div style={{ marginBottom: '2.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <div style={{ width: '56px', height: '56px', borderRadius: '14px', background: 'linear-gradient(135deg, #ef4444 0%, #dc2626 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 8px 16px rgba(239, 68, 68, 0.3)', animation: 'pulse-icon 2s infinite' }}>
            ...
            {errorMessage && (
                <div style={{ ...glassStyle, background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.3)', color: '#ef4444', display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem', padding: '1rem 1.5rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                    <div style={{ ...glassStyle, padding: '1rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div style={{ position: 'relative', flex: 1 }}>
                            <input
                                ...
                                style={{ background: 'var(--bg-glass)', border: 'var(--border-glass)', borderRadius: '0.75rem', padding: '0.6rem 1rem 0.6rem 2.5rem', color: 'var(--text-main)', outline: 'none', width: '300px' }}
                            />
                    ...
                    {loading ? (
                        ...
                        filteredHearings.map(hearing => {
                            ...
                            return (
                                <div key={hearing.id} style={{ ...glassStyle, padding: '1.5rem', display: 'flex', gap: '1.5rem', alignItems: 'center', border: hearing.isLive ? '1px solid rgba(239, 68, 68, 0.4)' : 'var(--border-glass-strong)', background: hearing.isLive ? 'linear-gradient(135deg, rgba(239, 68, 68, 0.05) 0%, rgba(255, 255, 255, 0.05) 100%)' : 'var(--bg-glass-strong)' }}>
                                    <div style={{ width: '70px', height: '70px', borderRadius: '15px', background: isToday ? 'rgba(74, 222, 128, 0.1)' : 'rgba(99, 102, 241, 0.1)', border: isToday ? '1px solid rgba(74, 222, 128, 0.3)' : '1px solid rgba(139, 92, 246, 0.2)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                    ...
                                    <div style={{ flex: 1 }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                                            ...
                                            {hearing.isLive && (
                                                <span style={{ padding: '0.25rem 0.75rem', borderRadius: '1rem', fontSize: '0.65rem', fontWeight: '800', background: '#ef4444', color: 'white', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                                    ...
                                    <button
                                        ...
                                        disabled={!joinable}
                                        style={{ background: hearing.isLive ? 'linear-gradient(135deg, #ef4444 0%, #dc2626 100%)' : joinable ? 'linear-gradient(135deg, #6366f1 0%, #4f46e5 100%)' : 'rgba(100, 116, 139, 0.2)', color: joinable ? 'white' : 'var(--text-secondary)', border: 'none', borderRadius: '0.75rem', padding: '0.75rem 1.5rem', fontWeight: '700', fontSize: '0.9rem', cursor: joinable ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', gap: '0.5rem', boxShadow: joinable ? '0 4px 12px rgba(99, 102, 241, 0.3)' : 'none' }}
                                    >
                    ...
                    <div style={glassStyle}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '0.5rem', marginBottom: '1rem' }}>
                            ...
                            {weekDates.map((date, i) => {
                                ...
                                return (
                                    <div key={i} style={{ height: '35px', borderRadius: '8px', background: isDateToday ? 'rgba(99, 102, 241, 0.2)' : (hasHearing ? 'rgba(76, 209, 55, 0.1)' : 'rgba(255, 255, 255, 0.03)'), border: isDateToday ? '1px solid rgba(99, 102, 241, 0.4)' : (hasHearing ? '1px solid rgba(76, 209, 55, 0.2)' : '1px solid transparent'), color: isDateToday ? '#818cf8' : (hasHearing ? '#4cd137' : 'var(--text-secondary)'), display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.85rem', fontWeight: '700' }}>{date.getDate()}</div>
                                );
                            })}
                        </div>
