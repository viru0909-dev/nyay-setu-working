...
import { Video, Calendar, Clock, Users, Loader2, AlertCircle, ExternalLink } from 'lucide-react';
...
export default function LiveHearing() {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(fetchTodaysHearings, 30000);
    ...
    const fetchTodaysHearings = async () => {
        try {
            ...
            const response = await axios.get(`${API_BASE_URL}/api/judge/todays-hearings`, {
    ...
    const joinHearing = (hearingId, videoRoomId) => {
        // Navigate to conduct hearing page with video room
        navigate(`/judge/conduct?hearingId=${hearingId}&roomId=${videoRoomId}`);
    };
    ...
    const isUpcoming = (scheduledDate) => {
        ...
        return diffMinutes > 0 && diffMinutes <= 30; // Within next 30 minutes
    };
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.5rem' }}>
                    ...
                    <div>
                        ...
                        <p style={{ fontSize: '1rem', color: 'var(--text-secondary)', margin: 0 }}>
                            Today's scheduled virtual court sessions • {new Date().toLocaleDateString('en-IN', { weekday: 'long', month: 'long', day: 'numeric' })}
                        </p>
                    </div>
                </div>
            </div>
                            ...
                            <div key={hearing.id} style={{
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', gap: '2rem', flexWrap: 'wrap' }}>
                                    <div style={{ flex: 1, minWidth: '300px' }}>
                                        ...
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1.5rem', marginTop: '1rem', fontSize: '0.95rem', color: 'var(--text-secondary)' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                                <Users size={16} />
                                                <span>Case ID: {hearing.caseId.substring(0, 13)}...</span>
                                            </div>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                                <Clock size={16} />
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', alignSelf: 'center' }}>
                                        {(live || upcoming) && (
                                            <button
                                                onClick={() => joinHearing(hearing.id, hearing.videoRoomId)}
                                                style={{
                                                    padding: '1rem 2rem',
                                                    background: live ? 'linear-gradient(135deg, #ef4444 0%, #dc2626 100%)' : 'linear-gradient(135deg, #6366f1 0%, #4f46e5 100%)',
                                                    color: 'white',
                                                    border: 'none',
                                                    borderRadius: '0.75rem',
                                                    fontWeight: '700',
                                                    cursor: 'pointer',
                                                    boxShadow: live ? '0 4px 20px rgba(239, 68, 68, 0.4)' : '0 4px 20px rgba(99, 102, 241, 0.4)',
                                                    display: 'flex',
                                                    alignItems: 'center',
                                                    gap: '0.75rem',
                                                    fontSize: '1rem',
