...
export const caseAPI = {
  ...
  submitDraft: (id, draftContent) =>
    api.post(`/api/v1/cases/${id}/submit-draft`, { draftContent }),
  reviewDraft: (id, approved, comments) =>
    api.post(`/api/v1/cases/${id}/review-draft`, { approved, comments }),
  ...
  deliverVerdict: (id, verdictDetails) =>
    api.post(`/api/v1/cases/${id}/deliver-verdict`, { verdictDetails }),
};
...
export const documentAPI = {
  ...
  downloadCertificate: (id) =>
    api.get(`/api/v1/documents/${id}/certificate`, { responseType: "blob" }),
};
...
export const documentGenerateAPI = {
  ...
  download: (data) =>
    api.post("/api/documents/generate/download", data, {
      responseType: "blob",
    }),
};
...
export const hearingAPI = {
  ...
  complete: (id, notes) =>
    api.put(`/api/v1/hearings/${id}/complete`, { judgeNotes: notes }),
...
export const vakilFriendAPI = {
  ...
  startCaseSession: (caseId) =>
    api.post(`/api/v1/vakil-friend/case/${caseId}/start`),
  sendMessage: (sessionId, payload) =>
    api.post(`/api/v1/vakil-friend/chat/${sessionId}`, payload),
  completeSession: (sessionId) =>
    api.post(`/api/v1/vakil-friend/complete/${sessionId}`),
  getSession: (sessionId) =>
    api.get(`/api/v1/vakil-friend/session/${sessionId}`),
  ...
  analyzeDocument: (caseId, file, sessionId = null) => {
    ...
    return api.post(
      `/api/v1/vakil-friend/case/${caseId}/analyze-document`,
      formData,
    );
  },
  ...
  analyzeDocumentForSession: (sessionId, file) => {
    ...
    formData.append("file", file);
    return api.post(
      `/api/v1/vakil-friend/session/${sessionId}/analyze-document`,
      formData,
    );
  },
  ...
  getCaseDiary: (caseId) =>
    api.get(`/api/v1/vakil-friend/case/${caseId}/diary`),

  // Verify diary entry integrity
  verifyDiaryEntry: (entryId) =>
    api.get(`/api/v1/vakil-friend/diary/${entryId}/verify`),
};

// Case Assignment API (Auto-assign judges, lawyer selection)
export const assignmentAPI = {
  autoAssign: (caseId) =>
    api.post(`/api/v1/assignments/case/${caseId}/auto-assign`),
  assignLawyer: (caseId, lawyerId) =>
    api.post(`/api/v1/assignments/case/${caseId}/assign-lawyer`, { lawyerId }),
...
export const caseAssignmentAPI = {
  ...
  proposeLawyer: (caseId, lawyerId) =>
    api.post(`/api/v1/cases/${caseId}/propose-lawyer`, { lawyerId }),
  respondToProposal: (caseId, status) =>
    api.post(`/api/v1/cases/${caseId}/respond-proposal`, { status }),
  getPendingCases: () => api.get("/api/v1/cases/pending-assignment"),
  getJudgeWorkload: () => api.get("/api/v1/cases/judge-workload"),
  takeCognizance: (caseId, judgeId) =>
    api.post(`/api/v1/cases/${caseId}/take-cognizance`, { judgeId }),
  updateSummons: (caseId, served) =>
    api.post(`/api/v1/cases/${caseId}/update-summons`, { served }),
  updateDocumentStatus: (caseId, status) =>
    api.post(`/api/v1/cases/${caseId}/document-status`, { status }),
};
...
export const judgeAPI = {
  ...
  aiChat: (message, caseId) =>
    api.post("/api/v1/judge/ai-chat", { message, caseId }),
  getAICaseSummary: (caseId) =>
    api.get(`/api/v1/judge/case/${caseId}/ai-summary`),
  scheduleHearingAI: (prompt) =>
    api.post("/api/v1/judge/hearings/schedule-ai", { prompt }),
};
...
export const lawyerAPI = {
  ...
  generateDraft: (caseId, template) =>
    api.post("/api/v1/lawyer/draft", { caseId, template }),
  saveDraft: (caseId, draft) =>
    api.post("/api/v1/lawyer/draft/save", { caseId, draft }),
};

// Central Brain API
export const brainAPI = {
  chat: (message, sessionId = null) =>
    api.post("/api/v1/brain/chat", { message, sessionId }),
  analyzeCase: (query) => api.post("/api/v1/brain/analyze-case", { query }),
  getSuggestedDocuments: (caseDetails) =>
    api.post("/api/v1/brain/suggest-documents", { caseDetails }),
};
...
export const policeAPI = {
  ...
  verifyFir: (id, formData) =>
    api.post(`/api/v1/police/fir/${id}/verify`, formData),
  getStats: () => api.get("/api/v1/police/stats"),
  getPendingFirs: () => api.get("/api/v1/police/fir/pending"),
  updateFirStatus: (id, status, reviewNotes) =>
    api.put(`/api/v1/police/fir/${id}/status`, null, {
      params: { status, reviewNotes },
    }),
  health: () => api.get("/api/v1/police/health"),
  startInvestigation: (id) =>
    api.post(`/api/v1/police/investigation/${id}/start`),
  submitInvestigation: (id, findings) =>
    api.post(`/api/v1/police/investigation/${id}/submit`, { findings }),
  ...
  uploadEvidence: (id, formData) =>
    api.post(`/api/v1/police/investigation/${id}/evidence`, formData),
  generateAiSummary: (id) =>
    api.get(`/api/v1/police/investigation/${id}/summary`),
  draftCourtSubmission: (id) =>
    api.get(`/api/v1/police/investigation/${id}/draft-submission`),
};
...
export const caseTransitionAPI = {
  ...
  policeSubmitToCourt: (caseId, officerId, officerName) =>
    api.post(`/api/v1/cases/transition/${caseId}/submit-to-court`, {
      officerId,
      officerName,
    }),
  ...
  lawyerSaveDraft: (caseId, lawyerId, lawyerName, draftContent) =>
    api.post(`/api/v1/cases/transition/${caseId}/save-draft`, {
      lawyerId,
      lawyerName,
      draftContent,
    }),
  ...
  litigantApproveDraft: (caseId, litigantId, litigantName) =>
    api.post(`/api/v1/cases/transition/${caseId}/approve-draft`, {
      litigantId,
      litigantName,
    }),
  litigantRejectDraft: (caseId, litigantId, litigantName, reason) =>
    api.post(`/api/v1/cases/transition/${caseId}/reject-draft`, {
      litigantId,
      litigantName,
      reason,
    }),
  ...
  judgeTakeCognizance: (caseId, judgeId, judgeName) =>
    api.post(`/api/v1/cases/transition/${caseId}/take-cognizance`, {
      judgeId,
      judgeName,
    }),
  judgeAdvanceStage: (caseId, judgeId, judgeName) =>
    api.post(`/api/v1/cases/transition/${caseId}/advance-stage`, {
      judgeId,
      judgeName,
    }),

  // SYSTEM: Summons served
  markSummonsServed: (caseId) =>
    api.post(`/api/v1/cases/transition/${caseId}/summons-served`),

  // Get case health
  getCaseHealth: (caseId) =>
    api.get(`/api/v1/cases/transition/${caseId}/health`),
};
